...
class DesignAgent(BaseAgent):
    ...
    def _init_design_tools(self) -> None:
        """Initialize integrations with design tools."""
        try:
            self.figma = FigmaIntegration(api_key=settings.FIGMA_API_KEY)
            ...
            self.analytics = GoogleAnalyticsIntegration(property_id=settings.GA_PROPERTY_ID)
            
            self.design_tools = {
                DesignTool.FIGMA: self.figma,
                # Add other tools as needed
            }
            logger.info("Design tools initialized successfully")
        except Exception as e:
            logger.error(f"Failed to initialize design tools: {str(e)}")
